...
public class MOTableBuilder {
	...
	public MOTableBuilder addColumnType(int syntax, MOAccess access) {
		colTypeCnt++;
		columns.add(new MOColumn<Variable>(colTypeCnt, syntax, access));
		return this;
	}
	...
	public MOTable build() {
		DefaultMOTable dfTable = new DefaultMOTable(tableRootOid, indexDef,
				columns.toArray(new MOColumn[0]));
		MOMutableTableModel model = (MOMutableTableModel) dfTable.getModel();
